...
export default function StatisticsPage() {
  ...
  // Ключи уровней (как на других страницах)
  ...
  const pieData = [
    { name: t('stats_page.chartCompleted'), value: stats?.week.completed || 0, color: '#6366f1' },
    { name: t('stats_page.chartTotal'), value: Math.max(0, (stats?.week.total || 0) - (stats?.week.completed || 0)), color: '#f1f5f9' },
  ];

  const lvlIndex = Math.min((user?.level || 1) - 1, LEVEL_KEYS.length - 1);
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title={t('statistics')} 
        subtitle={t('stats_page.subtitle')} 
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard 
          label={t('today')} 
          value={`${stats?.today.rate || 0}%`} 
          icon="📅" 
          color="brand" 
          trend={`${stats?.today.completed}/${stats?.today.total} ${t('habits').toLowerCase()}`} 
        />
        <StatCard 
          label={t('thisWeek')} 
          ...
          icon="📊" 
          color="green" 
          trend={`${stats?.week.completed} ${t('completed').toLowerCase()}`} 
        />
        <StatCard 
          label={t('thisMonth')} 
          value={`${stats?.month.rate || 0}%`} 
          icon="🗓️" 
          color="orange" 
          trend={`${stats?.month.completed} ${t('completed').toLowerCase()}`} 
        />
        <StatCard 
          label={t('stats_page.totalHabits')} 
          value={stats?.totalHabits || 0} 
          icon="✅" 
          color="brand" 
          trend={t('stats_page.activeHabits')} 
        />
      </div>

      {/* Level card */}
      <div className="card p-6 bg-gradient-to-br from-brand-500 to-brand-700 text-white border-0 animate-slide-down">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-brand-200 text-sm mt-1">{user?.xp} XP {t('profile_page.totalXp')}</p>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Weekly bar chart - Только выполненные задания */}
        <div className="card p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={stats?.weeklyData || []} barSize={32}>
              ...
              />
              {/* Оставили только выполненные, добавили анимацию */}
              <Bar 
                dataKey="completed" 
                fill="#6366f1" 
                radius={[6,6,0,0]} 
                name={t('stats_page.chartCompleted')}
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
              ...
              <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" fill="#6366f1" fontSize={22} fontWeight="bold">
                {stats?.week.rate || 0}%
              </text>
            </PieChart>
          </ResponsiveContainer>
